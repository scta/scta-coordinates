...
<xsl:variable name="height" select="number($yBottomMax) - number($yTopMin)"/>
    ...
    <height><xsl:value-of select="$height"/></height>
        <percent><xsl:value-of select="$percent"/></percent>
    <iiif><xsl:value-of select="concat(round($percent * number($topLeftX)), ',', round($percent * number($yTopMin)), ',', round($percent * $width), ',', round($percent * $height))"/></iiif>
    ...
    <xsl:variable name="adjustedYTopMin" select="if (number($yTopMin) gt 20) then (number($yTopMin) - 20) else $yTopMin"/>
        ...
        <iiifAdjusted><xsl:value-of select="concat(round($percent * number($adjustedTopLeftX)), ',', round($percent * number($adjustedYTopMin)), ',', round($percent * ($width + 65)), ',', round($percent * ($height + 40)))"/></iiifAdjusted>
